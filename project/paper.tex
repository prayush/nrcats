...
\newcommand{\PennState}{\affiliation{Institute for Gravitation and the Cosmos, Department of Physics, The Pennsylvania State University, University Park, PA 16802, USA}} %
\newcommand{\VSM}{\affiliation{Department of Physics, Vivekananda Satavarshiki Mahavidyalaya, Manikpara, Jhargram, West Bengal 721513, India}} %
\begin{document}
...
\author{Aditya Vijaykumar\,\orcidlink{0000-0002-4103-0666}}\CITA{}
\author{Vaishak Prasad\,\orcidlink{0000-0001-6712-2457}}\PennState{}
\author{Md Arif Shaikh\,\orcidlink{0000-0003-0826-6164}}\VSM{}
...
The successful simulation of binary black hole (BBH) mergers in 2005 marked a watershed moment in computational astrophysics, resolving decades of theoretical and numerical challenges~\cite{Pretorius:2005gq, Campanelli:2005dd, Baker:2005vv, Szilagyi:2014fna, Lehner:2001wq, Arnowitt:1962hi, York:1971hw, Pretorius:2004jg, Shibata:1999hn}. In the years immediately following these breakthroughs, numerical relativity (NR) evolved rapidly from a proof-of-principle endeavor—capable of producing only a handful of short, marginally stable waveforms—into a robust, high-precision discipline. Advances in gauge conditions (such as the BSSN~\cite{Shibata:1995we, Baumgarte:1998te, Campanelli:2006fy} and generalized harmonic formulations~\cite{Alcubierre:2005gh, Pretorius:2007jn}), constraint-damping formulations, and scalable computational architectures~\cite{Loffler:2011ay, Bruegmann:2006ulg, Sperhake:2006cy, Bernuzzi:2009ex, Kidder:2016hev} enabled codes to routinely evolve BBH systems through inspiral, merger, and ringdown with exquisite accuracy~\cite{MacFadyen:1998vz, Ruffini:2009hg, Campanelli:2007cga, Gonzalez:2007hi}.

As the first robust simulations became available, early community-wide efforts laid the foundational groundwork for integrating NR into the nascent data analysis ecosystem. The NINJA-1 project \cite{Aylott:2009ya} pioneered the injection of these initial NR waveforms into simulated detector noise to evaluate the performance of early gravitational-wave search pipelines. NINJA-2 \cite{Ajith:2012az} extended this initiative by utilizing longer, more accurate waveforms to test the impact of NR errors on parameter estimation and early analytical model calibration. Subsequently, the Numerical Relativity and Analytical Relativity (NRAR) collaboration \cite{Hinder:2013oqa} focused on benchmarking the leading analytical waveform models of the era against a consolidated, multi-group set of NR simulations. While these landmark efforts successfully established the critical role of NR in data analysis, they largely predated the systematic, large-scale exploration of the parameter space.

Over the last decade, the field transitioned from computing isolated benchmark waveforms to executing massive parameter-space explorations, culminating in the production of comprehensive, high-volume public waveform databases by multiple independent collaborations. Generating these dense catalogs required immense computational infrastructure and careful coordination of initial parameters to systematically blanket previously sparse regions of the binary parameter space, such as high mass ratios, extreme spins, and precessing configurations. Chief among these efforts are the SXS catalog \cite{Boyle:2008zua, Boyle:2019kee}, produced by the Spectral Einstein Code (SpEC), which now contains thousands of waveforms; the RIT catalog \cite{Healy:2020vre}, produced by the LazEv code; and the Georgia Tech / UT Austin MAYA catalog \cite{Jani:2016wkt}, produced by the MayaKranc code. Each of these catalogs covers overlapping regions of the binary parameter space and is relied upon by the gravitational-wave data analysis and waveform modeling communities as an indispensable ground truth.

The practical importance of these modern, densely sampled NR catalogs cannot be overstated. Gravitational-wave detection and parameter estimation pipelines (such as PyCBC \cite{Weaving:2023fji}, LALSuite \cite{Wette:2020air}, and Bilby \cite{Ashton:2018jfp, Romero-Shaw:2020owr, Veitch:2014wba}) extensively use NR waveforms as high-fidelity injection signals and as calibration targets. Furthermore, analytical waveform models — including effective-one-body (EOB) frameworks~\cite{Buonanno:1998gg, Buonanno:2000ef, Pan:2011gk, Babak:2016tgq, Taracchini:2014xjs, Bohe:2016gbl}, phenomenological (IMRPhenom) families~\cite{Ajith:2007qp, Husa:2015iqa, Khan:2015jqa, Pratten:2020fqn, Pratten:2020ceb}, and NR surrogate models — are directly calibrated and validated against these numerical datasets. A systematic bias or unquantified error in any particular catalog will therefore propagate directly into model calibrations and, through them, into the parameter estimation for every observed gravitational-wave event. Quantifying this bias is not merely an academic exercise: it sets a fundamental floor on the accuracy of waveform models and, ultimately, limits our ability to extract precise astrophysical and cosmological information from current and future gravitational-wave observations~\cite{Isi:2020gwy, Berti:2018vdi, Romero-Shaw:2020owr, Gabbard:2017lja, Dax:2021tsq, Acernese:2015gua, Blanchet:2002av, Damour:2016abl, Bini:2024icd, Barkett:2019sxy, Ruiz:2021gsv, Baiotti:2016qnr, Duez:2009yz, Liu:2020slm, Schutz:1986gp, Holz:2005df, Barack:2014csr, Anninos:1994gp, Davis:1971gg, Chandrasekhar:1975zza, LIGOScientific:2017ycc, LIGOScientific:2021usb, KAGRA:2018plz, KAGRA:2013rdx}.

However, conducting the rigorous, direct inter-catalog cross-validation that this sensitivity demands is primarily obstructed by a fundamental difficulty: \textbf{parameter-space mismatch}. Different NR codes reach their initial conditions through distinct theoretical formalisms and numerical procedures. For example, the SXS catalog uses the Spectral Einstein Code (SpEC) \cite{Kidder:2000yq, Scheel:2006gg}, which evolves the spacetime utilizing dual-coordinate frames and dynamic excision to maintain high accuracy \cite{Scheel:2006gg, Hemberger:2012jz}, alongside powerful constraint damping techniques \cite{Szilagyi:2009qz}. SpEC constructs initial data by solving the Extended Conformal Thin-Sandwich (XCTS) equations using multi-domain pseudospectral methods \cite{Pfeiffer:2002wt}, followed by an iterative procedure to precisely tune orbital parameters and reduce initial eccentricity to astrophysically relevant levels \cite{Pfeiffer:2007yz, Buonanno:2010yk}. In contrast, both the RIT (LazEv) and MAYA (MayaKranc) catalogs generally employ the moving-punctures framework, constructing initial data by solving the Bowen-York momentum constraints~\cite{Bowen:1980yu, Brandt:1997tf}—commonly relying on tools like the TwoPunctures solver \cite{Ansorg:2004ds}. Because these distinct mathematical formulations define the initial orbital separation, spin magnitudes, spin orientations, and center-of-mass velocity differently, matching the physical initial data precisely across codes is computationally expensive and never exact. As a result, for two catalog simulations $h^A(\boldsymbol{\theta}_i)$ and $h^B(\boldsymbol{\theta}_j)$ that nominally represent the same binary, the residual parameter-space distance $|\boldsymbol{\theta}_i - \boldsymbol{\theta}_j|$ can easily dominate over the intrinsic numerical errors one hopes to measure \cite{hannam2009}. In the worst case, $\|h^A(\boldsymbol{\theta}_i) - h^B(\boldsymbol{\theta}_j)\|$ is comparable to $\|h^A(\boldsymbol{\theta}_i) - h^A(\boldsymbol{\theta}_k)\|$ for a nearby point $\boldsymbol{\theta}_k$ in the same catalog — i.e., the cross-catalog difference is dominated by the interpolation error of the catalog itself.

\textbf{Surrogate models as comparison mediators.} The advent of high-accuracy NR surrogate models opens a new avenue for cross-catalog validation. Surrogate models are fast, data-driven interpolants built directly from a set of high-fidelity NR training waveforms using reduced-order modeling techniques~\cite{Blackman:2015pia, Field:2013cfa, Blackman:2017pcm}. Unlike traditional analytical waveform models (e.g., EOB or phenomenological models) which rely on physical approximations and fitting formulas to capture full waveform phenomenology, surrogates perform basis decomposition and empirical interpolation mode-by-mode. As a result, within their training domain, they can faithfully reproduce the original NR waveforms—complete with sub-dominant modes and complex precession dynamics—with errors comparable to the intrinsic numerical truncation errors of the training simulations themselves. 

A leading example is NRSur7dq4 \cite{Varma:2019csw, Varma:2018aht}, a fully precessing surrogate trained on 1528 SpEC simulations. Several of its technical characteristics dictate the structure of our comparative framework. First, the model interpolates waveform coefficients decomposed into spin-weight $-2$ spherical harmonics up to $\ell=4$ in a co-precessing frame, providing direct access to higher-order harmonics but strictly fixing the source-frame orientation convention to that of its SpEC training data. Second, NRSur7dq4 is trained exclusively on quasi-circular ($e=0$) binaries with mass ratios $q \le 4$ and spin magnitudes $\chi \le 0.8$. This strict domain limitation means that evaluating the surrogate on highly eccentric or boundary-case parameters probes its extrapolation behavior, allowing us to use eccentric configurations as a built-in negative control for our comparative metrics. 
...
\subsection{Waveform Multipoles and Coordinate Ambiguities}
...
Alternatively, collaborations extract radiation via the Newman-Penrose Weyl curvature scalar $\Psi_4$~\cite{Newman:1963vrk, Teukolsky:1973ha, Newman:1965tw}, defined by projecting the Weyl tensor $C_{\alpha\beta\gamma\delta}$ onto a null tetrad $\{l^\mu, n^\mu, m^\mu, \bar{m}^\mu\}$:
...
\subsection{Gauge Transformations: Source-Frame Rotations and BMS Supertranslations}
...
\textbf{Source-Frame Rotations.} Let $\mathbf{F}_C$ (catalog frame) and $\mathbf{F}_S$ (surrogate frame) be related by a rigid rotation $R \in \mathrm{SO}(3)$, as illustrated in Fig.~\ref{fig:coordinate_frames_rotation}. Under this rotation, the multipoles transform and mix mode-by-mode via the Wigner $D$-matrices~\cite{OShaughnessy:2012iol}:
...
% Script: project/scripts/generate_coordinate_rotation_figure.py
% Results on disk: project/figs/coordinate_frames_rotation.png
\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{figs/coordinate_frames_rotation.png}
\caption{Illustration of the rigid rotation $R \in \mathrm{SO}(3)$ relating the numerical catalog source frame $\mathbf{F}_C$ (blue axes) and the surrogate model's coprecessing frame $\mathbf{F}_S$ (red axes). Because different codes employ disparate conventions for initializing their coordinate triads, comparing extracted multipoles requires systematically optimizing the waveform match over these unphysical rigid frame rotations to properly align the coordinate systems before evaluating phase and amplitude differences.}
\label{fig:coordinate_frames_rotation}
\end{figure}

\textbf{BMS Supertranslations.} Beyond rigid rotations, the full BMS symmetry group at $\mathcal{I}^+$ introduces supertranslations, which represent direction-dependent retarded time translations of the form $u \to u - \alpha(\theta,\phi)$. Expanding the slicing function in terms of ordinary spherical harmonics~\cite{Thornburg:2003sf}, $\alpha(\theta,\phi) = \sum_{j,k} \alpha_{jk}\, Y_{jk}(\theta,\phi)$, the transformed multipoles to first order in the supertranslation coefficients $\alpha_{jk}$ are given by:
...
\subsection{Waveform Agreement Metrics}
...
\section{Waveform Catalogs and Surrogate Model}

\subsection{Numerical Relativity Catalogs and Simulation Classification}

To structure the comparison across catalogs, we classify every simulation according to its spin geometry and orbital eccentricity.  Let $\chi_\perp = \sqrt{\chi_{1x}^2 + \chi_{1y}^2 + \chi_{2x}^2 + \chi_{2y}^2}$ be the total in-plane spin magnitude and $e$ be the reference eccentricity from the catalog metadata.  We define six categories (summarized in Table~\ref{tab:categories_counts}) with thresholds $\varepsilon_\chi = 0.001$ and $\varepsilon_e = 0.005$.

% Script: project/scripts/generate_catalog_statistics.py
% Results on disk: project/results/catalog_statistics.md
\begin{table*}[t]
\centering
\caption{Simulation categories defined by spin geometry and orbital eccentricity, along with total simulation counts, calibration counts (SXS only), and counts within the NRSur7dq4 prior volume (shown in parentheses) for each catalog.}
\label{tab:categories_counts}
\begin{tabular}{llcccc}
\toprule
Category & Formal conditions & SXS & RIT & MAYA \\
\midrule
(a) Non-spinning eccentric & $\chi_\perp < \varepsilon_\chi$, $|\chi_{1z}| + |\chi_{2z}| < \varepsilon_\chi$, $e > \varepsilon_e$ & 206 (0, 0) & 499 (0) & 74 (0) \\
(b) Non-spinning quasi-circular & $\chi_\perp < \varepsilon_\chi$, $|\chi_{1z}| + |\chi_{2z}| < \varepsilon_\chi$, $e \leq \varepsilon_e$ & 177 (60, 123) & 54 (35) & 34 (25) \\
(c) Aligned-spin eccentric & $\chi_\perp < \varepsilon_\chi$, $|\chi_{1z}| + |\chi_{2z}| \geq \varepsilon_\chi$, $e > \varepsilon_e$ & 21 (0, 0) & 231 (0) & 117 (0) \\
(d) Aligned-spin quasi-circular & $\chi_\perp < \varepsilon_\chi$, $|\chi_{1z}| + |\chi_{2z}| \geq \varepsilon_\chi$, $e \leq \varepsilon_e$ & 687 (282, 464) & 541 (199) & 40 (26) \\
(e) Precessing eccentric & $\chi_\perp \geq \varepsilon_\chi$, $e > \varepsilon_e$ & 30 (0, 0) & 117 (0) & 303 (0) \\
(f) Precessing quasi-circular & $\chi_\perp \geq \varepsilon_\chi$, $e \leq \varepsilon_e$ & 3043 (1389, 1962) & 437 (25) & 67 (49) \\
\midrule
...
\textbf{Catalog counts.} Table~\ref{tab:categories_counts} shows both the total simulation count in each catalog and the number of simulations within the NRSur7dq4 prior volume ($q \leq 4$, $|\chi_{1,2}| \leq 0.8$, and $e = 0$) shown in parentheses. For the SXS catalog, the parenthesis additionally includes the number of simulations used directly for surrogate calibration, using the format: $N_{\rm total}~(N_{\rm calibration}, N_{\rm prior})$ after metadata filtering.
...
\textbf{NRSur7dq4 calibration sub-classification.} A key feature of the SXS catalog is that 1,731 of its simulations were used as training data for the NRSur7dq4 surrogate~\cite{Varma:2019csw}: 60 in category (b), 282 in category (d), and 1,389 in category (f).  All calibration simulations are quasi-circular ($e = 0$) by the surrogate's training design.  Categories (a), (c), and (e) contain no calibration simulations.  This stratification is recorded in the \texttt{catalog\_organization/sxs\_classification.json} file as a per-simulation boolean flag, propagated into the results CSV, and used to split the SXS analysis into calibration (in-sample) and non-calibration (out-of-sample) subsets.
...
\subsection{NRSur7dq4}

NRSur7dq4 \cite{Varma:2019csw, Varma:2018aht} is a fully precessing surrogate model trained on a dense grid of 1528 SpEC simulations at mass ratios $q = m_1/m_2 \in [1, 4]$ and spin magnitudes $|\boldsymbol{\chi}_{1,2}| \leq 0.8$. It provides all mode coefficients up to $\ell = 4$ (excluding $(5,5)$) as complex numpy arrays $h_{\ell m}$ in dimensionless $r h_{\ell m}/M$ units — following the standard spin-weight $-2$ spherical harmonic convention used by \texttt{WaveformModes.get\_mode()}, so no convention conversion is required, only amplitude scaling and time rescaling. The model accepts:
...
\subsection{Mode Extraction and Scaling}
...
\begin{table*}[t]
\centering
\caption{Per-mode match $\mathcal{F}$ for the four pilot simulations evaluated against NRSur7dq4. Modes marked with $\dagger$ vanish by symmetry, rendering their match numerically meaningless. Modes marked with $\ddagger$ represent anomalous amplitude discrepancies (discussed in the text).}
\label{tab:pilot_match}
\begin{tabular}{ccccc}
\toprule
Mode & 0001 ($q$=1, ns) & 0005 ($q$=1, spin) & 0169 ($q$=2, ns) & 0162 ($q$=2, spin) \\
\midrule
(2,+2) & 0.9940 & 0.9933 & 0.9947 & 0.9931 \\
(2,+1) & — $\dagger$ & \textbf{0.9966} & 0.9513 & 0.350 $\ddagger$ \\
(3,+3) & — $\dagger$ & 0.9548 & 0.9968 & 0.9844 \\
(4,+4) & 0.902 & 0.9886 & 0.9965 & 0.9778 \\
(5,+5) & N/A & N/A & N/A & N/A \\
(3,+2) & 0.9932 & 0.9924 & 0.9581 & \textbf{0.569 $\ddagger$} \\
(4,+3) & — $\dagger$ & 0.9744 & 0.9596 & 0.406 $\ddagger$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\centering
\caption{Phase difference per GW cycle $\Delta\Phi/{\rm cycle}$ [rad] and number of NR cycles $N_{\rm cyc}$ (in parentheses) for the four pilot simulations. Modes marked with $\dagger$ vanish by symmetry.}
\label{tab:pilot_phase}
\begin{tabular}{ccccc}
\toprule
Mode & 0001 ($q$=1, ns) & 0005 ($q$=1, spin) & 0169 ($q$=2, ns) & 0162 ($q$=2, spin) \\
\midrule
(2,+2) & 0.051 (42 cyc) & 0.004 (44 cyc) & 0.005 (43 cyc) & 0.005 (47 cyc) \\
(2,+1) & — $\dagger$ & 0.049 (25 cyc) & 0.005 (24 cyc) & 0.025 (26 cyc) \\
(3,+3) & — $\dagger$ & 0.045 (67 cyc) & 0.019 (65 cyc) & 0.001 (71 cyc) \\
(4,+4) & 0.983 (71 cyc) & 0.309 (89 cyc) & 0.334 (86 cyc) & 0.240 (95 cyc) \\
(5,+5) & — & — & — & — \\
(3,+2) & 0.163 (43 cyc) & 0.004 (44 cyc) & 0.003 (43 cyc) & 0.004 (47 cyc) \\
(4,+3) & — $\dagger$ & 0.236 (69 cyc) & 0.001 (65 cyc) & 0.004 (71 cyc) \\
\bottomrule
\end{tabular}
\end{table*}
...
% Script: project/scripts/plot_batch_results.py
% Results on disk: project/figs/fig2a_phasediff_vs_params.png
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/fig2a_phasediff_vs_params.png}
\caption{Cycle-averaged phase difference ($\Delta\Phi/{\rm cycle}$) evaluated against mass ratio, effective spin, individual spins, and initial eccentricity across all catalog simulations. This metric exposes secular phase drifts that the time/phase-maximized match metric may hide, revealing that high eccentricity and extreme mass ratios drive the largest systematic phase accumulation errors.}
\label{fig:phasediff_vs_params}
\end{figure*}

\subsection{Batch Comparison of Non-Spinning and Aligned-Spin Systems}

% Script: project/scripts/batch_aligned_catalogs.py
% Results on disk: project/results/batch_aligned_{all,maya,rit,sxs}.csv
...
% Script: project/scripts/plot_batch_results.py
% Results on disk: project/figs/fig1a_mismatch_vs_params.png
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/fig1a_mismatch_vs_params.png}
\caption{Scatter plots showing the noise-weighted match $\mathcal{F}$ per mode against key source parameters: mass ratio ($q$), effective spin ($\chi_{\rm eff}$), individual spins, and initial eccentricity. Evaluated between the NR catalogs and the NRSur7dq4 surrogate, this landscape highlights that the dominant $(2,2)$ mode typically exceeds $0.95$ agreement, while sub-dominant modes display larger spreads. Eccentricity rapidly degrades the match, exposing both numerical systematics and the surrogate's quasi-circular limitations.}
\label{fig:mismatch_vs_params}
\end{figure*}

% Script: project/scripts/plot_batch_results.py
% Results on disk: derived from project/results/batch_aligned_{sxs,rit,maya}.csv
\begin{table*}[t]
\centering
\caption{Per-mode match statistics (median / 10th percentile) evaluated over quasi-circular non-spinning and aligned-spin systems (categories b and d) for the SXS, RIT, and MAYA catalogs.}
\label{tab:batch_match_stats}
\begin{tabular}{cccc}
\toprule
...
\end{table*}
% Script: project/scripts/plot_batch_results.py
% Results on disk: project/figs/fig2b_phasediff_vs_params_qc.png
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/fig2b_phasediff_vs_params_qc.png}
\caption{Cycle-averaged phase difference ($\Delta\Phi/{\rm cycle}$) evaluated against mass ratio and spin parameters restricted to the quasi-circular subset. Removing eccentric simulations clarifies that for purely quasi-circular systems, the phase error remains generally well-constrained below $0.1$ rad/cycle, except for boundary cases at high mass ratios or extreme spins where surrogate extrapolation fails.}
\label{fig:phasediff_vs_params_qc}
\end{figure*}

\subsubsection{Quasi-circular Systems}
...
The RIT $(2,2)$ distribution is broader, with a median of 0.9231 and a 10th percentile of 0.494. The cumulative distribution function (CDF, Figure~\ref{fig:mismatch_cdf_combined}) reveals a bimodal structure featuring a high-match peak near $\mathcal{F} \sim 0.99$ and a low-match population extending below 0.5. As shown in Figure~\ref{fig:mismatch_vs_params}, the low-match RIT simulations are concentrated at high $|\chi_{\rm eff}|$ and $q \sim 4$. Because these fall near the boundary of the surrogate's training domain, increased phase mismatch is expected.

% Script: project/scripts/plot_batch_results.py
% Results on disk: project/figs/fig3b_phasediff_cdf.png
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/fig3b_phasediff_cdf.png}
\caption{Cumulative distribution functions (CDFs) of the phase difference per cycle across the SXS, RIT, and MAYA catalogs. SXS waveforms overwhelmingly demonstrate $\Delta\Phi/{\rm cycle} \sim 0.01$ rad/cycle for the dominant mode, whereas RIT and MAYA show larger systematic tails. This suggests that the match deficits observed in these catalogs are partially driven by cumulative inspiral phase errors rather than pure amplitude discrepancies.}
\label{fig:phasediff_cdf}
\end{figure*}

% Script: project/scripts/plot_batch_results.py
% Results on disk: project/figs/fig3a_mismatch_cdf.png
\begin{figure*}[htbp]
\centering
(a) \includegraphics[width=0.8\textwidth]{figs/fig3a_mismatch_cdf.png}\\
(b) \includegraphics[width=0.8\textwidth]{figs/fig5_sxs_cal_mismatch_cdf.png}
\caption{Cumulative distribution functions (CDFs) of waveform mismatches. \textbf{(a)} The noise-weighted match $\mathcal{F}$ for the $(2,2)$ and sub-dominant modes across quasi-circular subsets of the catalogs. The CDFs reveal a tight high-match peak for SXS, contrasted with a bimodal structure for RIT and a broader tail for MAYA, indicating that out-of-domain extrapolation (e.g., at high mass ratios) disproportionately impacts the surrogate's accuracy on the latter catalogs. \textbf{(b)} Mismatches ($1 - \mathcal{F}$) separating the SXS quasi-circular dataset into simulations used for NRSur7dq4 calibration (in-sample) versus independent testing (out-of-sample). The near-perfect agreement (mismatch $\sim 10^{-3}$) for the calibration set validates the surrogate's interpolation fidelity, while the extended lower tail for the test set identifies regions where the sparse training grid limits generalization accuracy.}
\label{fig:mismatch_cdf_combined}
\end{figure*}
...
Sub-dominant mode behavior varies substantially across catalogs, as seen in Table~\ref{tab:batch_match_stats} and Figures~\ref{fig:mismatch_vs_params}, \ref{fig:phasediff_cdf}, and \ref{fig:mismatch_cdf_combined}. For the SXS catalog, the median matches for $(3,3)$ (0.987) and $(4,4)$ (0.982) are high, whereas the $(4,3)$ median is lower (0.671) and the odd-$m$ modes $(2,1)$ and $(3,2)$ show wide spreads. This wide spread reflects the binary-symmetry mechanism: for equal-mass, non-spinning systems, odd-$m$ modes vanish, rendering their matches numerically ill-defined.
...
The phase-difference metric provides additional diagnostic insights (Figures~\ref{fig:phasediff_vs_params} and \ref{fig:phasediff_vs_params_qc}). For quasi-circular SXS systems, the $(2,2)$ phase difference has a median of 0.01 rad/cycle, which is well below the threshold of 0.1 rad/cycle typically considered acceptable for parameter estimation. The $(4,4)$ mode exhibits the largest phase errors (median $\sim$0.3 rad/cycle), consistent with the results in Section IV.B and the sparser \texttt{NRSur7dq4} training in the $\ell = 4$ sector. The RIT and MAYA catalogs show systematically larger phase differences for several modes, suggesting that their match deficits in Table~\ref{tab:batch_match_stats} are partially attributable to cumulative phase error rather than pure amplitude discrepancy.

% Script: project/scripts/analyze_figure4_hotspots.py
% Results on disk: project/figs/fig4_mismatch22_heatmap_qc.png
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/fig4_mismatch22_heatmap_qc.png}
\caption{Parameter-space heatmaps of the $(2,2)$ mode mismatch ($1 - \mathcal{F}$) in the $(q, \chi_{\rm eff})$ plane. The SXS and RIT catalogs share a pronounced hotspot of degradation near the $q \approx 4$ domain boundary, indicating the surrogate's interpolation limits. However, isolated hotspots at lower mass ratios in the RIT catalog point to code-specific systematic numerical differences distinct from surrogate errors.}
\label{fig:mismatch22_heatmap_qc}
\end{figure*}

\subsubsection{Effect of eccentricity}

We analyze the quantitative dependence of mismatch and phase errors on orbital eccentricity across the catalogs. Figure~\ref{fig:mismatch_vs_params} maps the broad mode-by-mode landscape, and our focused analysis on eccentricity reveals systematic catalog dependencies.

% Script: project/scripts/plot_eccentricity_dependence.py
% Results on disk: project/figs/fig_eccentricity_mismatch.png and project/figs/fig_eccentricity_phase.png
\begin{figure*}[htbp]
\centering
(a) \includegraphics[width=0.8\textwidth]{figs/fig_eccentricity_mismatch.png}\\
(b) \includegraphics[width=0.8\textwidth]{figs/fig_eccentricity_phase.png}
\caption{Effect of initial orbital eccentricity $e$ on waveform agreement. \textbf{(a)} Mode-by-mode mismatch ($1 - \mathcal{F}$). For $e \lesssim 0.005$, mismatch is dominated by numerical noise, but for $e > 0.005$, mismatch escalates according to a deterministic power law, confirming that the quasi-circular surrogate fundamentally fails to capture eccentric modulations (like periastron advance), with higher-order modes degrading faster due to amplified frequency modulation. \textbf{(b)} Mode-by-mode phase difference per cycle $\Delta\Phi/{\rm cycle}$. The phase drift remains negligible ($< 0.05$ rad/cycle) in the quasi-circular regime but increases to $> 1.0$ rad/cycle for highly eccentric systems. This illustrates that unmodeled radial oscillations and periastron precession introduced by high eccentricity manifest as severe secular phase accumulation errors over the long inspiral.}
\label{fig:eccentricity_combined}
\end{figure*}
...
\subsection{Precessing Comparison Pipeline}
...
\subsection{SO(3) Frame Rotation Optimization}
...
\subsection{Model Dependency Analysis}
...
\subsubsection{SEOBNRv4PHM Results}
...
% Script: project/scripts/plot_seobnrv4phm_results.py
% Results on disk: project/figs/seobnrv4phm_mismatch_vs_q.png
\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{figs/seobnrv4phm_mismatch_vs_q.png}
\caption{Mismatch against mass ratio ($q$) for quasi-circular systems when evaluated against the SEOBNRv4PHM phenomenological model instead of NRSur7dq4. The complete disappearance of the $q \approx 4$ boundary degradation confirms that the inter-catalog mismatches observed previously were artifacts of the surrogate's out-of-sample extrapolation, demonstrating that the underlying NR codes agree well beyond the surrogate's strict training domain.}
\label{fig:seobnrv4phm_mismatch_vs_q}
\end{figure}
% Script: project/scripts/plot_seobnrv4phm_results.py
% Results on disk: project/figs/seobnrv4phm_mismatch_vs_ecc.png
\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{figs/seobnrv4phm_mismatch_vs_ecc.png}
\caption{Mismatch evaluated against the SEOBNRv4PHM model as a function of initial eccentricity. Similar to the surrogate model, the quasi-circular SEOBNRv4PHM framework sharply degrades for $e_0 \gtrsim 0.02$. This provides a negative control demonstrating that the mismatch captures true physical deviation due to unmodeled eccentric modulations rather than arbitrary numerical noise.}
\label{fig:seobnrv4phm_mismatch_vs_ecc}
\end{figure}
